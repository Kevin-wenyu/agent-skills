...
│   ├── book-research-planning/
│   ├── wechat-topic-gen/
│   ├── wechat-research/
│   ├── wechat-deepen/
│   ├── wechat-proofreading/
│   ├── wechat-proofreading-workspace/  # proofreading 的 eval 测试记录，不是 skill 本身
│   ├── wechat-write-article/       # 调用名仍是 write-article，见下方说明
│   ├── wechat-newsletter-digest/   # 调用名仍是 newsletter-digest
│   └── wechat-architecture-diagram/ # 调用名仍是 architecture-diagram
...
`skills/` 下所有公众号相关 skill 目录都带 `wechat-` 前缀，写书流水线的带 `book-` 前缀——**这个前缀标的是"属于哪条流水线"，不是"谁写的"**（2026-08-20之前带过 `kevin-` 前缀，去掉了：这个仓库本来就是单人维护，标所有权不携带任何信息量，反而让人以为跟内容有别的关系）。前缀存在的唯一目的是让同一条流水线在 `ls skills/ | sort` 里排在一起，不用只靠索引表分辨——没有前缀的话，`write-article`/`newsletter-digest`/`architecture-diagram` 这类名字会散落在字母表各处，这是踩过的真实坑。

3 个跟着源头同步的 skill（`write-article`/`newsletter-digest`/`architecture-diagram`）目录名同样套 `wechat-` 前缀，但 SKILL.md 文件本身（frontmatter 的 `name` 字段、脚本调用等）**不改**，仍然是 `write-article`/`newsletter-digest`/`architecture-diagram`，因为这才是它们的真实调用名，改了就跟源头对不上、也没法触发。也就是说：**目录名管排序和归类，SKILL.md 里的 `name` 管实际调用**，这两件事是分开的，别搞混。
...
|---|---|---|---|---|
| `book-research-planning` | 同左 | 写书流水线第1阶段：把模糊的书想法整理成确认过的章节结构 + 逐章调研笔记 | 已建，未验证，未注册为可调用 skill | 这个仓库里直接改 |
| `wechat-topic-gen` | 同左 | 公众号选题生成：给一个话题，产出3-4个真正有差异的方向，带优劣分析，交用户选 | 已建，未验证，未注册为可调用 skill | 这个仓库里直接改 |
| `wechat-research` | 同左 | 公众号深度调研：选题定了之后按信息源优先级搜索，边搜边存，产出结构化简报 | 已建，未验证，未注册为可调用 skill | 这个仓库里直接改 |
| `wechat-deepen` | 同左 | 拔高：检查文章核心判断有没有Kevin自己的专业经验支撑，没有就问本人要，不编造，不是扩写/堆案例 | 已建，未验证，未注册为可调用 skill | 这个仓库里直接改 |
| `wechat-proofreading` | 同左 | 初稿写完后的AI味+细节审校，不查事实（write-article已管），独立子agent跑 | 已建，用skill-creator Eval模式验证过一轮（2026-08-19，`evals/`目录），未注册为可调用skill——Kevin决定先不装，等在真实公众号文章上跑过验证再说 | 这个仓库里直接改 |
...
- [x] 配图 → 发布 → 3 个配图 skill / `newsletter-digest`（编译类走这个而不是`write-article`，生产在用）

（曾经有一个泛化打包分享的独立分支`kevin-wechat-skill/`，2026-08-20决定不做通用分发，已删除——这个仓库现在只服务 Kevin 自己的流水线，不维护对外分发的版本。git 历史里还能找到，commit `57cdb35`。）

## 怎么改一个 skill

**新建**：用 `skill-creator` skill 起草，别手写——它定义好了 SKILL.md 结构、eval/test 流程、description 优化方法。新 skill 放在 `skills/<name>/`，kebab-case，跟 frontmatter 里的 `name` 对齐。同一条流水线的 skill 共享一个功能前缀（公众号流水线是 `wechat-`，写书流水线是 `book-`）——前缀标的是"属于哪条流水线"，不是"谁写的"。一个新的、不相关的项目起一条新的前缀，同样的道理。SKILL.md 控制在 500 行以内，大参考材料放 `references/`，可执行脚本放 `scripts/`。

**改一个跟着源头同步的 skill**（`write-article`、`newsletter-digest`、`architecture-diagram`）：先去源头（`~/.claude/skills/<name>`）改，改完手动 `cp -r` 过来这个仓库对应的 `skills/wechat-<name>/` 目录再提交——**目录名要重新套一层 `wechat-` 前缀，但 `cp` 过来的 SKILL.md 内容本身（包括 frontmatter 的 `name` 字段）原样不动，不要跟着改名**。不能反过来在这个仓库里单方面改——那边才是天天在用的权威版本，这边的副本改了不会生效，还会造成两边不一致。没有自动检测漂移的机制，靠人工重新对比。
...
```bash
ln -s ~/Documents/GitHub/skills_creator_project/skills/wechat-proofreading ~/.claude/skills/wechat-proofreading
```
