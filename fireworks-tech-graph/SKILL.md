...
| 捷径思路 | 为什么不行 | 正确做法 |
|---|---|---|
| "语法验证通过就行" | 语法正确≠逻辑正确；并行流程极容易画成串行 | 逐节点、逐箭头检查语义，不只跑 rsvg-convert |
| "箭头方向看起来对" | 箭头方向错误是最常见的逻辑 bug，肉眼不够可靠 | 写出每条箭头的语义（谁触发谁、数据流向哪里）再画 |
| "超过15个节点压缩一下就好" | 压缩导致信息密度超标，读者理解成本翻倍 | 超过15个元素必须拆成多张图，不得压缩 |
| "先生成再调整布局" | 布局返工成本极高，结构性错误很难修 | 生成前先列出节点、分层关系和箭头语义 |
| "SVG 文件输出了就完成" | 未经 rsvg-convert 验证的 SVG 可能在目标环境渲染失败 | 必须跑 `rsvg-convert` 看到 ✓ Valid 才算完成 |
| "配色随便选一个" | 浅色系对比度不足会在手机微信压缩后变成一片灰 | 填充色不浅于 #C5C2F5/#B8E8D4，文字用 #1A1A1A |
| "多个箭头交叉没关系" | 交叉箭头让读者分不清流向，是最常见的图表 bug | 交叉必须加 jump-over arc（5px radius），详见 Layout Rules |
