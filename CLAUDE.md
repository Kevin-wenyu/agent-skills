...
- **Self-authored skills** get a `kevin-` prefix (e.g. `kevin-book-research-planning`), so they're recognizable at a glance against mirrored/third-party skills. Self-authored skills belonging to the same pipeline additionally share a secondary prefix — `kevin-book-*`, `kevin-wechat-*` — so that subset sorts together. A new, unrelated project gets its own secondary prefix the same way rather than overloading an existing one.
- **This prefix does NOT group a whole pipeline in the flat `skills/` folder** once mirrored skills are part of it — `write-article`/`newsletter-digest` keep their real names (see below) and sort wherever their name falls alphabetically, nowhere near `kevin-wechat-*`. Directory sort order is not the pipeline index; README's per-pipeline tables are. Don't claim or imply that naming alone groups a mixed pipeline — verify against the actual `ls skills/ | sort` output before writing that claim anywhere.
