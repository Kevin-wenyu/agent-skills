...
- `kevin-wechat-topic-gen/` — second skill, first stage of a planned WeChat-article publishing workflow (separate from the book pipeline): 选题生成 → 深度调研 → 三遍审校 → 配图 → 发布前检查 (Options, then save-as-you-go research, then Checklist, then Options+Integration+Checklist combo, then Checklist again — modeled on the 花叔 huashu-* examples in the guide, chapter 1 and chapter 9). Options pattern: given a rough topic, produces 3-4 genuinely different directions with title/angle/outline/effort estimate/honest pros-and-cons, and waits for the user to pick rather than choosing itself. Same one-at-a-time build order as the book pipeline — only this first stage exists; 个人素材匹配 and 跨平台分发 were considered but deferred until the core 5 are validated in use. Note: the eventual `kevin-wechat-publish-check` stage can only produce a publish-ready draft + checklist — actually clicking "publish" is a permission-gated action that always requires per-use user confirmation regardless of skill instructions, so no skill in this repo can automate that final click away.
