...
- **Naming convention:** every self-authored skill in this repo is prefixed `kevin-` (e.g. `kevin-book-research-planning`), matching the guide's own advice to give personally-authored skills a consistent personal prefix (花叔 uses `huashu-`) — this makes them recognizable at a glance and distinct from official/third-party skills (`docx`, `pdf`, `pptx`, etc.) once the skill list grows. Skills belonging to the book-writing pipeline additionally share a `kevin-book-` secondary prefix (e.g. `kevin-book-research-planning`, and future `kevin-book-writing`, `kevin-book-assembly`, `kevin-book-versioning`, `kevin-book-publish`) so the 5 pipeline stages sort and scan together, independent of unrelated future skills.
