...
- `architecture-diagram/` — mirrored, complete (`resources/template.html` included). Dark-themed HTML+SVG architecture diagrams, used for more complex system/infra diagrams than `fireworks-tech-graph`'s default style handles.
- `excalidraw-diagram-generator/` — mirrored, **incomplete**: only `SKILL.md` copied, the `references/`, `templates/`, and `scripts/` it references weren't present in the source either. Generates `.excalidraw` JSON files for hand-drawn-style diagrams.
...
Flat layout (no category subfolders) is deliberate, following the reference project [addyosmani/agent-skills](https://github.com/addyosmani/agent-skills): categorize via this table / prefix, not via nested directories — keeps every skill a uniform one-level-deep path.
