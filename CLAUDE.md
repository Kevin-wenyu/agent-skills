...
- `kevin-wechat-topic-gen/`, `kevin-wechat-research/`, `kevin-wechat-proofreading/`, `kevin-wechat-image/`, `kevin-wechat-publish-check/` — 5-stage WeChat-article publishing workflow (separate from the book pipeline): 选题生成 → 深度调研 → 三遍审校 → 配图 → 发布前检查 (Options, then save-as-you-go research, then Checklist, then Options+Integration+Checklist combo, then Checklist again — modeled on the 花叔 huashu-* examples in the guide, chapter 1 and chapter 9). All 5 stages are scaffolded with full content (not stubs), but none has been validated against a real article yet — build order deviated from the usual one-at-a-time rule at the user's explicit request ("先搭建好" — scaffold everything first, validate together later), so expect heavier revision once real use surfaces gaps, especially in `kevin-wechat-image` (needs the user's actual image-gen tool / image host / aesthetic preferences filled in on first real use) and `kevin-wechat-publish-check` (排版坑 list is a starting guess, not yet confirmed against the actual WeChat editor). `kevin-wechat-publish-check` only ever produces a publish-ready draft + checklist — actually clicking "publish" is a permission-gated action that always requires per-use user confirmation regardless of skill instructions, so no skill in this repo can automate that final click away. 个人素材匹配 and 跨平台分发 were considered but deferred.
