...
- **Every skill in `skills/` gets a secondary-prefixed directory name so its pipeline sorts together in the flat folder** — `kevin-book-*` for the book pipeline, `kevin-wechat-*` for the WeChat pipeline, applied to *every* member of that pipeline regardless of whether it's self-authored or synced from a real source. This was changed after the original rule ("only self-authored skills get the prefix") turned out to not actually group anything — `ls skills/ | sort` showed `write-article`/`newsletter-digest`/`architecture-diagram` scattered across the whole alphabet, nowhere near `kevin-wechat-*`, because the exempted mirrored skills were most of the pipeline. A new, unrelated project gets its own secondary prefix the same way rather than overloading an existing one.
- **For skills synced from a real source (`write-article`, `newsletter-digest`, `architecture-diagram`), the directory name and the SKILL.md frontmatter `name` are deliberately different** — directory is `kevin-wechat-write-article` (grouping/sorting only), frontmatter `name` stays `write-article` (the real invocation name, must match the canonical source verbatim or it breaks). This is the one place in this repo where directory name ≠ frontmatter `name` — don't "fix" that mismatch, it's intentional. Self-authored skills don't have this split: directory name and frontmatter `name` are the same value.
